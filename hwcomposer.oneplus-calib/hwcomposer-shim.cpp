...
struct ThisCallHookClass {
    int32_t hookSetPreferredColorModeInternal(const std::string &mode_string, bool from_client, int32_t *color_mode, int *dynamic_range) __asm__(SYMBOL_SetPreferredColorModeInternal);
};

int32_t ThisCallHookClass::hookSetPreferredColorModeInternal(const std::string &mode_string, bool from_client, int32_t *color_mode, int *dynamic_range) {
    ...
    auto err = std::invoke(origin_func, this, mode_string, from_client, color_mode, dynamic_range);
    if (err) return err;
    ALOGI("set sdm color mode %s, from client %d, loading calibration...", mode_string.c_str(), from_client);
