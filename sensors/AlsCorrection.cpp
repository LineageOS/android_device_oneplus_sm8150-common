...
void AlsCorrection::correct(float& light) {
    ...
    if (light - correction >= 0) {
        ...
        if (light - (correction * (((float) white_max_lux) / (red_max_lux + green_max_lux + blue_max_lux))) >= 0) {
            // all collected stock calibration values have red_max_lux + green_max_lux + blue_max_lux > white_max_lux
            // attempt to apply a scale here
            light -= correction * (((float) white_max_lux) / (red_max_lux + green_max_lux + blue_max_lux));
        } else if (light - correction > -4.0f) {
            // for low negative corrected values, make it positive
