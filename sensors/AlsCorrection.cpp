...
static const struct als_config default_conf = {
    .hbr = false,
...
static const struct als_config default_conf = {
    .hbr = false, // TODO this device is missing odm_feature_list, verify this
...
static struct als_config conf;

struct hysteresis_range {
    float middle;
    float min, max;
};

static const struct hysteresis_range default_hysteresis_ranges[] = {
    { 0, 0, 4 },
    { 7, 1, 12 },
    { 15, 5, 30 },
    { 30, 10, 50 },
...
static const int num_hysteresis_ranges = sizeof(default_hysteresis_ranges) / sizeof(hysteresis_range);
static struct hysteresis_range hysteresis_ranges[num_hysteresis_ranges];
...
void AlsCorrection::init() {
    ...
    for (int i = 0; i < num_hysteresis_ranges; i++) {
        hysteresis_ranges[i] = default_hysteresis_ranges[i];
        hysteresis_ranges[i].min /= conf.calib_gain * conf.sensor_inverse_gain[0];
        hysteresis_ranges[i].max /= conf.calib_gain * conf.sensor_inverse_gain[0];
    }
...
void AlsCorrection::process(Event& event) {
    ...
    */
    ...
    if (state.force_update
        ...
        // TODO: proper IPC
        // afaict oneplus has a thread in surfaceflinger taking screenshots
        // continuously, and doing the more expensive math on demand with
        // some kind of delay compensation
