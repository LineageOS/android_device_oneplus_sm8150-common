...
void AlsCorrection::init() {
    ...
    std::istringstream is(GetProperty("vendor.sensors.als_correction.bias", ""));
    is >> als_bias;
    ...
    max_brightness = get(BRIGHTNESS_DIR "max_brightness", 1023);
    ALOGV("Display maximums: R=%d G=%d B=%d W=%d",
        red_max_lux, green_max_lux, blue_max_lux, white_max_lux);
...
void AlsCorrection::correct(float& light) {
    ...
    static AreaRgbCaptureResult rgb_readout = { 0, 0, 0 };

    struct timespec now;
    clock_gettime(CLOCK_MONOTONIC, &now);
    if (now.tv_sec - lastScreenUpdate > 1) {
        if (service != nullptr && service->getAreaBrightness(&rgb_readout).isOk()) {
            lastScreenUpdate = now.tv_sec;
        } else {
            ALOGE("Could not get area above sensor");
        }
    } else {
        ALOGV("Capture skipped because interval not expired at %ld", now.tv_sec);
    }
